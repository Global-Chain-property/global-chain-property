    ...
    <header className="fixed top-0 left-0 w-full bg-white shadow-md z-50 transition-all duration-300">
      <div className="container mx-auto flex justify-between items-center py-4 px-6 md:px-10">
        ...
        <div className="text-2xl font-bold text-navy tracking-tight">
          GlobalChain <span className="text-gold">Property</span>
        </div>
        ...
        <nav className="hidden md:flex space-x-8">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href} 
              className="text-gray-600 hover:text-gold font-medium transition duration-300"
            >
              {link.name}
            </a>
          ))}
          ...
          <nav className="flex flex-col space-y-3">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-lg text-navy hover:text-gold font-medium py-2 border-b border-gray-100 last:border-b-0"
            ...
            <button 
              className="
                mt-4 w-full bg-gold text-white font-semibold 
                py-2.5 rounded-full
